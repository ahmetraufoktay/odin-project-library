...
function createBook(name,author,pages,color,read) {
    ...
    this.createBookBox = function() {
        ...
        removeBook.addEventListener('click', ()=> {
            ...
            let index = myLibrary.indexOf(this.name);
            myLibrary.splice(index,1);

            if (bookContainer.children.length === 0) {/* remove the bookshelf if the book we are removing is the last book */
...
submitBook.addEventListener("click",function(event) {
    ...
    const myBook = new createBook(nameValue,authorValue,pageValue,colorSVG,readValue);
...
/* preventing esc behaviour on modals */
